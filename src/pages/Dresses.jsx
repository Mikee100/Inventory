...
export default function Dresses() {
  ...
  useEffect(() => {
    fetch('https://inventory-backend-gpon.onrender.com/api/dresses')
      .then(res => res.json())
      .then(result => {
  ...
  const handleDeductStock = async (dress) => {
    ...
    try {
      const res = await fetch(`https://inventory-backend-gpon.onrender.com/api/dresses/${dress._id}/deduct`, {
        method: 'POST',
  ...
  const handleAddStock = async (dress) => {
    ...
    try {
      const res = await fetch(`https://inventory-backend-gpon.onrender.com/api/dresses/${dress._id}/add`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ quantity }),
      });
  ...
  <div className="fixed inset-0 bg-white bg-opacity-30 flex justify-center items-center z-50" style={{ backdropFilter: 'blur(8px)' }}>
          <div className="bg-white p-8 rounded-lg shadow-lg max-w-md w-full" onClick={e => e.stopPropagation()}>
            ...
            <form className="space-y-2" onSubmit={async e => {
              e.preventDefault();
              try {
                const res = await fetch(`https://inventory-backend-gpon.onrender.com/api/dresses/${editId}`, {
                  method: 'PUT',
                  headers: { 'Content-Type': 'application/json' },
